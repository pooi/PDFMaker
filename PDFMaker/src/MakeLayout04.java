...
public class MakeLayout04 {
	...
	public static void main(String[] args) throws DocumentException, IOException {
		...
		PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream("layout04.pdf"));
		
		document.setMargins(PageSize.A4.getWidth()/2 + padding, padding, padding, padding);
		
		float width = PageSize.A4.getWidth()/2 - padding*2;
...
//		Add Image
		...
		float imgWidth = PageSize.A4.getWidth()/2;
		float imgHeight = Math.round((float)height/imageList.size() - ((imageList.size()-1)/2));
...
//		    img.setAbsolutePosition(imgWidth + i*(imageList.size()/2), document.top()-imgHeight+padding);
		    img.setAbsolutePosition(0, imgHeight * i + i*(imageList.size()/2));
		    document.add(img);
		}
...
//	    Add reference
	...
	public static float getHeightEng(float width, String str, int size) throws DocumentException, IOException{
